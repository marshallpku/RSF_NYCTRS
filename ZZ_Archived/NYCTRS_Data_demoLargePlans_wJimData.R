...
#*********************************************************************************************************
#                      ## 1. Loading data from Jim Kampl  ####
#*********************************************************************************************************

# NYSTRS 

df_NYCsurvey <- read_xlsx(paste0("Inputs_data/", "Avg. Salary by Position-Age-Service.xlsx"), 
													sheet = "Table 1", 
													range = "A2:E122",
...
df_NYCsurvey %<>% 
	mutate(agegrp_lb = as.numeric(str_extract(agegrp, "^\\d{2}")),
				 ...
				 agegrp = str_replace_all(agegrp, fixed(" "), ""),
				 yosgrp = str_replace_all(yosgrp, fixed(" "), ""),
				 
				 agegrp = ifelse(agegrp == "66+", "65-69", agegrp),
				 yosgrp = ifelse(yosgrp == "30+", "30-34", agegrp)
				 
				 
				 
	) %>% 
	select(position, agegrp, yosgrp, age.cell, yos.cell, nactives, salary)
...
# 	geom_point()
# 
# df_NYCsurvey %>% filter(age.cell == 22)
...
df_NYCsurvey %<>% 
	...
	summarise(
		...
		yosgrp   = unique(yosgrp),
		nactives_sum = sum(nactives),
		salary   = sum(nactives * salary)/sum(nactives)) %>% 
	rename(nactives = nactives_sum)


df_NYCsurvey %<>% 
	gather("type", "value", -agegrp,-age.cell, -yosgrp, -yos.cell) %>% 
	mutate(plan = "NYCsurvey") %>% 
	select(plan, type, age.cell, agegrp, yos.cell, yosgrp, value)



#*********************************************************************************************************
#                      ## 1. Loading data of large plans   ####
#*********************************************************************************************************
...
df_large <- df_salary_large %>% filter(plan != "NYCsurvey")
...
#*********************************************************************************************************
#           # 3.3 Compare constructed scales with NYC survey data provided by Jim   ####
#*********************************************************************************************************

scale_nactives

df_NYCsurvey %>% filter(type == "nactives") %>% ungroup %>% 
	mutate(nactives_share_survey = value / sum(value))

  
scale_nactives %>% 
	left_join(df_NYCsurvey %>% filter(type == "nactives") %>% 
							ungroup %>% 
							mutate(nactives_share_survey = value / sum(value)) %>% 
							select(age.cell, yos.cell, nactives_share_survey)) %>% 
	gather(scaleType, value, -age.cell, -yos.cell) %>% 
	ggplot(aes(x = yos.cell, y = value, color = scaleType)) + theme_bw() +
	facet_wrap(~age.cell) + 
	geom_line()+
...
scale_nactives %>%
	...
	summarise(
		nactives_share_w = 100 * sum(nactives_share_w),
		nactives_share_s = 100 * sum(nactives_share_s),
		nactives_share_survey = 100 * sum(nactives_share_survey, na.rm = TRUE)
	) %>%
	left_join(df_age_NYCTRS %>% rename(nactives_share_TRS = nactives_share)) %>%
	gather(type, value,-age.cell,-plan) %>%
	ggplot(aes(x = age.cell, y = value, color = type)) + theme_bw() +
...
df_NYCsurvey %>%
	ungroup() %>% 
	spread("type", "value") %>% 
	mutate(sal.avg = sum(salary * nactives)/sum(nactives),
				 sal.scale_survey = salary / sal.avg) %>% 
	select(age.cell, yos.cell, sal.scale_survey)
...
scale_salary %>% 
	left_join(df1) %>%
	mutate(sal.scale_survey = na2zero(sal.scale_survey)) %>% 
	gather(scaleType, value, -age.cell, -yos.cell) %>% 
	group_by(scaleType, age.cell) %>% 
	mutate(value = value / mean(value)) %>% # all values standardized by age group mean 
	ggplot(aes(x = yos.cell, y = value, color = scaleType)) + theme_bw() +
...
bind_rows(df_salary_large, df_NYCsurvey %>% spread("type", "value")) %>%
	# filter(!plan %in% c("CALSTRS", "GATRS")) %>% 
	# filter(!plan %in% c("GATRS")) %>% # looks better to include GATRS
	group_by(plan, age.cell) %>% 
	summarise(salary = sum(salary * nactives) / sum(nactives),
