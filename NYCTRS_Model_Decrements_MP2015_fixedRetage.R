...
mortality_model_MP2015 %<>% 
	...
	mutate(
		# service retirees
		qxm_servRet_proj = qxm_servRet,
		qxm_disbRet_proj = qxm_disbRet
	) %>% 
...
#mortality_model_MP2015 %>% filter(year ==2017)
...
retRates_model_tiers %<>% 
	mutate(qxr_t4a = ifelse(age >= 60, 1, 0),
				 qxr_t4b = ifelse(age >= 60, 1, 0),
				 qxr_t4c = ifelse(age >= 60, 1, 0),
				 qxr_t4d = ifelse(age >= 60, 1, 0),
				 qxr_t6  = ifelse(age >= 60, 1, 0))
