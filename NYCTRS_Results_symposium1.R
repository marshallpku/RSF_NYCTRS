...
results_all %>% filter(runname == "multiTier_TDAamortAS_OYLM", sim == 0)
...
fig_singleReturns1_cumGeoMean <- 
	...
	geom_point() +
	geom_hline(yintercept = 7, linetype = 2)+
	scale_y_continuous(breaks = c(7, seq(-100,100, 2)) ) +
	scale_x_continuous(breaks = c(2016, seq(2020, 2045, 5), 2048)) + 
	scale_color_manual(values = c("black", "deepskyblue1", "firebrick1")) + 
	labs(title = fig.title   ,
			 subtitle = fig.subtitle,
...
fig.title    <- "Distribution of employer contribution rates across simulations11"
fig.subtitle <- "Assumption achieved: expected compound return = 7% (before TDA transfer)"
fig_ERCdist <- 
results_all %>% 
	filter(runname %in% "multiTier_TDAamortAS_OYLM", year >= 2019, sim > 0) %>%
	...
	scale_fill_gradient(low="red", high="pink") +
	scale_linetype_manual(values = c(1, 1, 1, 1, 1 ) ) +
	scale_shape_manual(values = rev(c(1,16, 15, 17, 2))) +
...
save_figure(fig_FRdisb,   w = 1.5*4, h = 1.3*4 )
save_figure(fig_ERCdist,  w = 1.5*4, h = 1.3*4 )
...
fig_shock1_FR <- 
	results_all %>% 
	filter(runname %in% c(runs_DF[1:2], "multiTier_TDAamortAS_OYLM"), sim == 0, year >= 2018) %>% 
	select(runname, year, FR_MA) %>% 
	mutate(runname.fct = factor(runname, levels = c(runs_DF[1:2], "multiTier_TDAamortAS_OYLM"), 
															labels = c(runs_DF_labels[1:2], "Baseline: Constant 7% return") )) %>% 
	ggplot(aes(x = year,
						 y = FR_MA,
						 color = runname.fct,
	...
	scale_color_manual(values = c("black", RIG.blue, "grey50", "red"),  name = NULL) +
...
save_figure(fig_shock1_ERC, w = 1.2*5, h = 1*5 )
save_figure(fig_shock1_FR,  w = 1.2*5, h = 1*5 )
...
save_figure(fig_ERChigh.fPolicy, w = 1.65*4, h = 1.3*4 )
