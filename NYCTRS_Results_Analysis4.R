...
runs_TDA_offset <- c("multiTier_TDAamortAS_OYLM_offset2p5",
                     "multiTier_TDAamortAS_OYLM_offset5"
...
runs_TDA_offset <- c("Addtional ERC of 2.5%   of payroll",
										 "Addtional ERC of 5% of payroll")
...
runs_all        <- c(runs_TDA, runs_TDA_OYLM, runs_TDA_alt, runs_RS, runs_DF, runs_fPolicy)
runs_all_labels <- c(runs_TDA_labels, runs_TDA_OYLM_labels, runs_TDA_alt_labels, runs_RS_labels, runs_DF_labels, runs_fPolicy_labels)
...
fig_TDA_alt_FR40less <- df_all.stch %>% 
	...
	scale_shape_manual(values = c(16, 17,18, 0, 1, 2, 3, 15),  name = "") +
	labs(title = fig.title,
			 subtitle = fig.subtitle,
			 x = NULL, y = "Probability") + 
	guides(color = guide_legend(keywidth = 1.5, keyheight = 3))+
